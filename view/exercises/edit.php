...
?>
			<form class="form-signin" action="index.php?controller=exercises&amp;action=edit&amp;exerciseId=<?=$exercise->getExerciseId()?>" method="POST">
				<?=i18n("ID")?>:
				<input disabled type="text" name="exerciseId" class="form-control" placeholder="<?=i18n("ID")?>" value="<?=$exercise->getExerciseId()?>" required>
				<?=i18n("Name")?>:
				<input type="text" name="exerName" class="form-control" placeholder="<?=i18n("Name")?>" value="<?=$exercise->getExerName()?>" required>
				<?=i18n("Type")?>:
